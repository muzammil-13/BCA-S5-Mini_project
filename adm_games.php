				...
				<div class="row">
					...
					<div class="col-lg-8">
						<div class="review-content text-box text-white">
							...
							<a class="read-more" href = "edit_game.php?gameid=<?php echo $row['idproduct'];?>">
									edit<img src="img/icons/double-arrow.png" alt="#"/>
							</a>
                            <?php echo "&nbsp&nbsp&nbsp&nbsp";
							$pid=$row['idproduct']?>
                            <button class="btn btn-danger" onclick="deleteProduct(<?php echo $pid; ?>)">Delete</button>
						</div>
					</div>
				</div>
			</div>
          ...
          $("#product_" + id).remove();
			...
			if ($num_pages > 1) {
				...
				for ($i = 1; $i <= $num_pages; $i++) {
					if ($i != $current_page) {
						echo '<a href="adm_games.php?s=' . (($display * ($i - 1))) . '&np=' . $num_pages .'">' . $i . '</a>';
					} else {
